...
Mas porque alguém iria querer um algoritmo de multiplicação mais rápido? Multiplicações em computadores já não sao quase instantâneas?

Bom, para números pequenos, de fato não faz sentido utilizar o algoritmo de Karatsuba. Inclusive, para números muito pequenos a quantidade de chamadas recursivas e outras operações do algoritmo de Karatsuba podem fazer sua complexidade ser superior à múltiplicação tradicional. Esse algoritmo começa a ser útil quando você estiver tratando números muito grandes, de mais de 100 dígitos! Para números maiores de por exemplo 570 dígitos, o algoritmo tradicional leva 324900 computações de um dígito, enquanto o algoritmo de Karatsuba leva aproximadamente 23332 computações equivalentes (operações entre números com um dígito).

Mas porque alguém iria querer fazer operações entre 2 números de 570 dígitos?

## Aplicações
Como não é o foco do handout, serei breve nessa seção.
...
Se sentiu interessado? fique a vontade para pesquisar mais aplicações desse algoritmo. Se quiser um lugar para começar, [aqui](https://dergipark.org.tr/en/download/article-file/2160206) seria um bom local.



## Algoritmo padrão

O algoritmo padrão, também conhecido como "multiplicação longa" ou "método escolar", é a forma como aprendemos a multiplicar no colégio. É aquela que multiplicamos cada dígito de um número pelo outro e somamos as linhas ao final. Digamos que, por exemplo, você tinha que multiplicar 23958233 por 5830 em uma prova do ensino médio, onde não era permitido o uso de calculadora. Você realizaria a operação da seguninte maneira (algoritmo):
```md
      1370
×     5830
—————————————
...
```
??? Checkpoint
Considerando agora uma operação atômica aquela que envolve estritamente 2 valores de dígitos únicos, qual é a complexidade da "multiplicação longa"? 
...
???
Bom, tecnicamente falando, levando em consideração que uma instrução atômica será qualquer operação entre 2 dígitos individuais, a complexidade da "multiplicação longa" será um pouco maior que $n^2$, pois temos que realizar n somas, referente aos digitos indiviuais multiplicados. Mas por simplificação, podemos considerar $n^2 + n$ como sendo $O(n^2)$. Para o restante do handout, se preocupe apenas com as multiplicações quando pensarmos em complexidade. Elas são mais custosas para o computador do que somas e subtrações.

## Fundamentos do algoritmo de Karatsuba

Já estamos na seção principal deste handout. Mas antes de continuarmos, me permita fornecer um pouco mais de história. Em 1960, o jovem Anatoly Karatsuba de 23 anos, enquanto atendia à um seminário do grande matemático [Andrei Kolmogorov](https://pt.wikipedia.org/wiki/Andrei_Kolmogorov) foi exposto à seguinte afirmação: É impossível fazer uma multiplicação em menos de $2n^2-1$ operações. Intrigado pela alegação, Karatsuba começou a pensar sobre o problema.

Menos de uma semana depois, Karatsuba descobre um algoritmo mais eficiente.

Vamos agora tentar entender como ele chegou à solução tão rapidamente. A primeira ideia que você tem que ter bem fixa em mente é que uma operação atômica é estritamente entre números de 1 dígito agora. Por simplificação, pense na complexidade apenas em função de multiplicações, como foi explicado no caso da multiplicação longa.

Você então, tem em mãos, dois números de n dígitos, mas só pode lidar com operações entre dígitos individuais. Você precisa "quebrar" seus números de alguma forma a obter 1 dígito.
...
Karatsuba teve que pensar numa forma inteligente de dividir os dígitos de um número. Dividir para conquistar!

É exatamente essa a caracterização do algoritmo, "Divide and Conquer".

Imagine uma multiplicação entre dois números de 2 dígitos que queremos multiplicar: 15 e 20. Vamos dividir ao meio cada um desses valores e chamaremos então:
$$ a = 1 $$
$$ b = 5 $$
$$ c = 2 $$
$$ d = 0 $$
??? Checkpoint
Escreva em função apenas de a e b o número 15 e em função de c e d o número 20
...
:::
???
Com isso acredito que fica trivial fazer o próximo exercício. Se preferir, não se preocupe em escrever no papel, esses checkpoints são para mantê-lo ativo mentalmente.
??? Checkpoint
Escreva a multiplicação $15*20$ em função de a, b, c, d:


::: Gabarito
    
$$15*20 = (a*10+b)*(c*10+d)$$
$$15*20 = 100ac + 10ad + 10bc + bd$$
ou também:
$$15*20 = 100ac+10(ad+bc)+bd$$
...
??? Checkpoint
Karatsuba notou o seguinte: Vamos computar $ac$ e $bd$. Será que não é possível agora escrever $ad+bc$ (a soma das computações restantes) sem fazer necessariamente mais 2 multiplicações ($ad$ e $bc$)? Tente chegar à essa conclusão, mas não se sinta mal se não conseguir e nem perca muito tempo aqui tentando. Lembre-se que até 1960, ninguém na história da humanidade conseguiu concluir isso.
...
É possível realizar a operação 15*20 com 3 multiplicações de dígitos indivíduais, ao invés de 4. Foi o seguinte que Anatoly Karatsuba conseguiu notar:
$$ad+bc = (a+b)(c+d)−ac−bd$$
Assim, ao invés de computar $ad$ e $bc$ (2 multiplicações) e somá-las, eu computo $(a+b)(c+d)$, que equivale à uma multiplicação. Disso eu consigo subtrair $ac$ e $bd$ (que eu já tenho calculado) e encontrar $ad+bc$, que é o que eu preciso para fazer $15*20 = 100ac+10(ad+bc)+bd$.
...
??? Checkpoint
Se vou multiplicar dois números A e B de n dígitos nós vamos primeiro quebrá-los, tanto A quanto B, em 2, como fizemos anteriormente (se não ficou claro, pense no número 143127, $a$ seria 143 e $b$ seria 127). Agora escreva A e B em função apenas de n (número de dígitos) e suas metades ($a$ e $b$ referentes ao número inteiro A e $c$ e $d$ referentes ao número inteiro B):
